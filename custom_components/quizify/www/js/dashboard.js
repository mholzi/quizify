...
(function() {
        ...
        switch (msg.phase) {
            ...
            case 'ANSWER_REVEAL':
                ...
                if (msg.round_summary && msg.round_summary.question_type === 'estimate' && msg.round_summary.estimate) {
                    // #895d: the number line was rebuilt and the frame
                    // around it was not — the board kept the previous
                    // round's counter and category, and the fun fact of
                    // the round being revealed never appeared at all.
                    // These are the same three writes the multiple-choice
                    // branch has always made.
                    renderRevealChrome(msg.round_summary, msg.round, msg.total_rounds);
                    if (els.answersGrid) els.answersGrid.innerHTML = '';
                    renderDashboardEstimateReveal(msg.round_summary.estimate);
                    showRevealFunFact(msg.round_summary.fun_fact);
    ...
    //
    // #895d split the frame (counter, category, question text) and the fun
    // fact out of it, because the estimate branch of the same case needs
    // both and had neither: it rendered the number line inside the
    // previous round's furniture and never showed the fun fact at all.
    function renderRevealChrome(rs, round, totalRounds) {
        els.funFact.classList.remove('visible');
    ...
    function showRevealFunFact(fact) {
        if (!fact) return;
    ...
    function renderRevealFromSnapshot(rs, round, totalRounds) {
        renderRevealChrome(rs, round, totalRounds);
        ...
        // #895d: the vote bars, which this path drew as bare tiles with no
        // distribution markup in them at all — so a television rebuilt
        // from a snapshot showed the correct answer and nothing about what
        // the room had picked. Same markup the live question grid embeds
        // up front (#151), same canonical index space, filled from the
        // snapshot's own `answer_distribution`.
        var distByIdx = {};
        (rs.answer_distribution || []).forEach(function(d) {
        ...
        els.answersGrid.innerHTML = answers.map(function(answer, i) {
            ...
            return '<div class="' + cls + '" data-index="' + i + '">' +
                ...
                '<div class="dashboard-answer-distribution" aria-hidden="true">' +
                    ...
                    '</div>' +
                    '<span class="dashboard-answer-percent">' + pct + '%</span>' +
                '</div>' +
                '</div>';
        }).join('');

        showRevealFunFact(rs.fun_fact);
    }
